...
bool XoppThumbnailer::create(const QString& path, int /*width*/, int /*heigth*/, QImage& img){
    ...
    if (!QFile::exists(thumbnailPath)){
        ...
        process->start("xournalpp-thumbnailer", {path, thumbnailPath});
        process->waitForFinished();
    }

    QFile thumbnailFile(thumbnailPath);
    img = QImage(thumbnailPath);
    return !img.isNull();
}
